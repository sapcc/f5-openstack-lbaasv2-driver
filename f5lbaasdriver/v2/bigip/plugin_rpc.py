...
class LBaaSv2PluginCallbacksRPC(object):
    ...
    def get_active_loadbalancers(self, context, env, group=None, host=None):
        ...
        with context.session.begin(subtransactions=True):
            ...
            for agent in agents:
                ...
                for lb in agent_lbs:
                    if lb.provisioning_status == neutron_const.ACTIVE:
                        loadbalancers.append(
                            {
                                'agent_host': agent['host'],
                                'lb_id': lb.id,
    ...
    def get_pending_loadbalancers(self, context, env, group=None, host=None):
        ...
        with context.session.begin(subtransactions=True):
            ...
            for agent in agents:
                ...
                for lb in agent_lbs:
                    if (lb.provisioning_status != neutron_const.ACTIVE and
                            lb.provisioning_status != neutron_const.ERROR):
                        loadbalancers.append(
                            {
                                'agent_host': agent['host'],
                                'lb_id': lb.id,
    ...
    def get_errored_loadbalancers(self, context, env, group=None, host=None):
        ...
        with context.session.begin(subtransactions=True):
            ...
            for agent in agents:
                ...
                for lb in agent_lbs:
                    if (lb.provisioning_status == neutron_const.ERROR):
                        loadbalancers.append(
                            {
                                'agent_host': agent['host'],
                                'lb_id': lb.id,
        ...
        with context.session.begin(subtransactions=True):
            try:
                ...
                if (lb_db.provisioning_status ==
                    ...
                    status = neutron_const.PENDING_DELETE
    ...
    def update_listener_status(self, context, listener_id=None,
                               provisioning_status=neutron_const.ERROR,
        ...
        with context.session.begin(subtransactions=True):
            try:
                ...
                if (listener_db.provisioning_status ==
                        neutron_const.PENDING_DELETE):
                    provisioning_status = neutron_const.PENDING_DELETE
                self.driver.plugin.db.update_status(
                    context,
                    models.Listener,
                    listener_id,
    ...
    def update_health_monitor_status(
            ...
            provisioning_status=neutron_const.ERROR, operating_status=None):
